...
class court_mighty(object):
    ...
    def __init__(self):  # u'初始化参数等等'
        ...
        self.url0 = 'http://xyzy.chinacourt.org/article/index/id/MzA1MDCwMDAwNCACAAA%3D/page/1.shtml'  # u'新余市中级法院'
        self.host = 'http://xyzy.chinacourt.org'
    ...
    def url_deal(self, n):
        # urlo = self.url
        url = 'http://xyzy.chinacourt.org/article/index/id/MzA1MDCwMDAwNCACAAA%3D/page/' + str(n) + '.shtml'
        self.n = n
        # print 'url',url
        return url
    ...
    def sub_page(self, te):  # 第一页链接过去的内容，第二子页处理
        ...
        sql = "insert into jx_xinyu3rd VALUES('%s','%s','%s','%s','%s')" % (self.link_id, self.title, \
                                                                       text, today,shi_qu)
    ...
    def sub_page1(self, te):  # 第一页链接过去的内容，第二子页处理
        bs = self.beaut_souping(te)
        # print bs
        try:
            divs = bs.find(class_='text')
        ...
        tr_list = divs.find('tbody').find_all('tr')
        for c, tr in enumerate(tr_list[3:]):
            td_list = tr.find_all('td')[:11]
            # print c,tr
            xh = td_list[0].text.strip()
            ktsj = td_list[1].text.strip()
            ah = td_list[2].text.strip()
            ay = td_list[3].text.strip()
            yg = td_list[9].text.strip()
            bg = td_list[10].text.strip()
            # print c,xh,ktsj,ah,ay,yg,bg
            fid = self.link_id + '_' + xh
            text = ktsj + ',' + u' 案号' + ah + u' 案由:' + ay + u' 原告:' + yg + u' 被告:' + bg
            # print c,text
            # text = divs.text.strip().replace('\n','').replace(' ','')
            # print shi_jian,fa_ting,an_hao,an_you,cheng_ban_ren,dang_shi_ren
            sql = "insert into jx_xinyu3rd VALUES('%s','%s','%s','%s','%s')" % (fid, self.title, \
                                                                           text, today,shi_qu)
            #print self.n, c, sql
            self.data_in(sql)
