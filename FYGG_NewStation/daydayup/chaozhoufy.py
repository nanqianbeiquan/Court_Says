...
# 	}
	...
	def __init__(self):  # u'初始化参数等等'
		...
		self.url0 = 'http://14.215.113.28:8180/courtweb/front/ggxxList/JJ0-splc-?gglx=kt' #u'潮州市中级人民法院'
	...
	def url_deal(self,n): #u'url改造工厂'
		...
		url = 'http://14.215.113.28:8180/courtweb/front/ggxxList/JJ0-splc-down-'+str(n)+'-?gglx=kt#up'
		print url
		return url
	...
	def text_analysis(self,bs):   #第一页网站数据抓取
		ta = bs.find_all('table')[6].find_all('tr')#.find_all('a')#[6].find_all('tr')#[5].find_all('td')[6].find_all('table')[1:-2]
		for i in range(len(ta)):
				tr_list = ta[i]#[0]
			# if i == 1:
				self.l = i
				linka = ta[i].a.get('href')
				link_text = ta[i].a.text
				self.an_hao = link_text.split(u'号')[0]+u'号'
				pubdate = ta[i].a.span.text
				link = self.host+linka
				# print i,link,link_text,self.an_hao
