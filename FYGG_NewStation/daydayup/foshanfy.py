...
# 	}
	...
	def __init__(self):  # u'初始化参数等等'
		...
		self.url0 = 'http://www.fszjfy.gov.cn/pub/court_7/sifagongkai/fayuangonggao/kaiting/' #u'佛山市中级人民法院'
		self.host = 'http://www.fszjfy.gov.cn'
	...
	def url_deal(self,n): #u'url改造工厂'
		urlo = self.url0
		url = 'http://219.130.221.114/dataImport/GetDate?method=GetAJXXs'
		print url
		return url
...
if __name__ == '__main__':
	...
	for i in range(1,3):
		# if i == 1:
			p = i
			url = cla.url_deal(i)
			data = cla.param_deal(i)
			headers = cla.headers_deal()
			t = cla.quest_postout(url,data,headers)
