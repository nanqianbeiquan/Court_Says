...
# 	}
	...
	def __init__(self):  # u'初始化参数等等'
		...
		self.url0 = 'http://www.zjcourt.gov.cn/zjzy/front/ggxxList/J80-splc-?gglx=kt' #u'湛江市中级人民法院'
		self.host = 'http://www.zjcourt.gov.cn'
	...
	def url_deal(self,n): #u'url改造工厂'
		...
		url = 'http://www.zjcourt.gov.cn/zjzy/front/ggxxList/J80-splc-down-'+str(n)+'-?gglx=kt#up'
		print url
		return url
	...
	def text_analysis(self,bs):   #第一页网站数据抓取
		ta = bs.find(class_='list').find_all('a')#[6].find_all('tr')#[5].find_all('td')[6].find_all('table')[1:-2]
		for i in range(len(ta)):
			# tr_list = ta[i].find_all('tr')#[0]
			# if i == 1:
				self.l = i
				linka = ta[i].get('href')
				link_text = ta[i].li.text
				self.an_hao = link_text.split(' ')[0]
				pubdate = ta[i].li.span.text
				link = self.host+linka
				# print i,link,self.an_hao
	...
	def sub_page(self,te):  #第一页链接过去的内容，第二子页处理
		...
		sql = "insert into zhanjiangfy Values('%s','%s','%s','%s','%s','%s','%s','%s')" %(self.an_hao,kai_shi_shi_jian,jie_shu_shi_jian,\
