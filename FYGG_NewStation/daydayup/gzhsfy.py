...
# 	}
	...
	def __init__(self):  # u'初始化参数等等'
		...
		self.url0 = 'http://www.gzhsfy.gov.cn/tszb.php?submitsearch=1=&page=1' #u'广州海事法院'
		self.host = 'http://www.gzhsfy.gov.cn/tszb.php'
	...
	def url_deal(self,n): #u'url改造工厂'
		urlo = self.url0
		url = 'http://www.gzhsfy.gov.cn/tszb.php?submitsearch=&page='+str(n)
		print url
		return url
	...
	def text_analysis(self,bs):   #第一页网站数据抓取
		ta = bs.find_all('table')[5].find_all('td')[6].find_all('table')[1:-2]
		# for i in range(len(ta)):
		# 	ta_list = ta[i]#.find_all('tr')
